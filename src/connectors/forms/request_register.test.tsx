...
describe("RequestRegisterForm", () => {
  ...
  beforeEach(() => {
    ...
    requestRegisterFormConnector = renderHook((props) => useRequestRegisterFormConnector(props), {
      initialProps: { loginAction },
      wrapper: QueryWrapperLight(queryClient),
    });

    screen = render(<RequestRegisterForm connector={requestRegisterFormConnector.result.current} />, {
    ...
    for (const [name, { form, responseStatus, expectErrors }] of Object.entries(forms)) {
      it(name, async () => {
        ...
        if (expectErrors.length === 0) {
          ...
          expect(screen.queryByText(/authenticator\.register:form\.success\.title/)).toBeNull();
          expect(screen.queryByText(/authenticator\.register:form\.success\.main/)).toBeNull();
          expect(screen.queryByText(/authenticator\.register:form\.success\.sub/)).toBeNull();
        }
      });
    }
  });
